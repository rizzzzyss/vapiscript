...
*/
    // ✅ buildPDFContent(): NO remote image, NO link — only BRD text + optional base64 upload
function buildPDFContent() {
  const collected = window.__vapiUi.collected || {};
  ...
  // ✅ Keep ONLY the user-uploaded base64 image (safe). Remove this block too if you want zero images.
  if (generatedBRD.userUploadedImageBase64) {
    html += `
      <div style="margin-bottom: 40px;">
...
// ✅ generatePDF(): same as before (safe + logs)
async function generatePDF() {
  ...
  // Force readable colors (in case BRD HTML has white text)
